...
def _get_search_domain(pms_property_ids, search=False, **post):
    domains = []
    search_exists = False
    if search:
        search_exists = True
    ...
    domain_fields = [("pms_property_id", "in", pms_property_ids)]
...
class PmsFolio(models.Model):
    ...
    def search_count_folios_pwa(self, search, **post):
        pms_property_ids = self.env.user.pms_pwa_property_ids.ids
        rdo = self.env["pms.folio"].search_count(
            _get_search_domain(pms_property_ids, search, **post)
        )
        return rdo
